...
// This allows integrations like saving files to a server.
...
window.systemHookDefaults = {
	...
	showOpenFileDialog: async ({ formats }) => {
		if (window.untrusted_gesture) {
			...
			show_error_message("Sorry, a file picker cannot be shown when using Eye Gaze Mode. You must click File > Open directly with the mouse, or press Ctrl+O on the keyboard.");
	...
	if (e.key === "Escape") { // Note: Escape handled above too!
		...
		switch (e.key.toUpperCase()) {
			...
			case "Y":
				...
				break;
			case "G":
				if (e.shiftKey == false) toggle_grid();
				break;
			case "F":
